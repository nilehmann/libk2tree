...
class BitArray {
 public:
  ...
  explicit BitArray(_Size length) :
      bits_(sizeof(T)*8),
      length_(length),
      data_(new T[Ceil(length_, bits_)]) {
    for (_Size i = 0; i < Ceil(length_, bits_); ++i)
      this->data_[i] = 0;
  }
  ...
  explicit BitArray(ifstream *in) :
      ...
      data_(LoadValue<T>(in, Ceil(length_, bits_))) {}
  ...
  void Save(ofstream *out) const {
    ...
    SaveValue(out, data_, Ceil(length_, bits_));
  }

  inline size_t GetSize() const {
    // bits_ + length + data_ + (ptr to data_)
    size_t size = sizeof(int) + sizeof(_Size);
    size += sizeof(_Size)*(Ceil(length_, bits_) + sizeof(_Size*);
    return size;
  }
